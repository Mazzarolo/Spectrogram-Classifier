...
def apply_snv(df):
    snv_normalized = df.copy()
    for col in df.columns:
        mean = df[col].mean()
        std = df[col].std()
        snv_normalized[col] = (df[col] - mean) / std
    return snv_normalized

def apply_gaussian_noise(data, sigma=0.1, relative_detach=True):
    if sigma == 0:
        return data

    scale = sigma * np.abs(data) if relative_detach else sigma
    noise = np.random.normal(0, scale, size=data.shape)
    return data + noise
...
for idx, (key, value) in enumerate(bacteria_files.items()):
    ...
    # y_columns = apply_snv(y_columns)
    y_columns = y_columns.apply(lambda row: apply_gaussian_noise(row, sigma=0.1), axis=1)
